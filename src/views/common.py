...
def page_nav(user: schema.User | None, old: bool = True):
    if old:
        ...
        return yahgl_common.page_nav(user)
    ...
    if old:
        return FlatGroup(
            ...
            _html.attrs(lang="en")(
                page_head(),
                body.attrs(
                    _class("bg-black text-white text-3xl h-screen " "lg:text-base")
                )(
                    page_nav(user),
                    div.attrs(
                        _class(
        ...
        return yahgl_common.page_root(
            user=user, child=page_nav(user, old=False)
        ).render()
...
def post_upload_form(old: bool = False):
    if old:
        return render(
            div.attrs(
        ...
        return yahgl_common.post_upload_form().render()
